...
fn test_grid_spacing_calculation_edge_cases() {
    ...
    for (length, resolution, expected) in test_cases {
        ...
        assert!((calculated - expected).abs() < 1e-10, 
                "Failed for length={}, resolution={}: expected {}, got {}", 
                length, resolution, expected, calculated);
    }
}
...
fn test_computational_consistency() {
    ...
    assert_eq!(spacing1, spacing2, "Grid spacing calculation should be deterministic");
    ...
    assert!((expected_total_length - length).abs() < 1e-10, 
